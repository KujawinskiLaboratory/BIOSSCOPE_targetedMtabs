function matchKujTargetedMetabolites_2021
%function matchKujTargetedMetabolites_2021
%This file will pull in data the 2021 data from the TSQ (two batches)
...
%3. The prior MATLAB *mat file with Kujawinski lab metabolites: 'BIOSSCOPE_metabolites_2016to2019.mat'
%4. The lookup table ('KujLab_metaboliteLookUpTable.xlsx') which holds the
%details on what SRM names were used in what year. 
%
%This will export a MATLAB *mat file, which can be built on with the next batch of data 
%Krista Longnecker
% 17 November 2023; updated 30 December 2023; 5 January 2024; 29 February 2024
clear all
close all
warning('off','MATLAB:table:RowsAddedExistingVars')

%2021 data are as follow:
...
ccnName{1} = '2021_1117 BIOSSCOPE temporal batch1\BIOS2021batch1_RP_TSQdata.2024.01.05_matrix.mat'; %where ccn = cruise, cast, niskin
ccnName{2} = '2021_1122 BIOSSCOPE temporal batch2\BIOS2021batch2_RP_TSQdata.2023.12.18_matrix.mat'; %where ccn = cruise, cast, niskin

%Required file #1: the 2021 data, load that in the loop that follows below
...
%Required filed #3: the MAT file with the 2016 to 2019 metabolites; metabolite concentrations are in pM
%% this file is now in the /dataFiles folder on GitHub, you may need a new path
eDir = '..\dataFiles';
eFile = 'BIOSSCOPE_metabolites_2016to2019.mat';
...
if ~isequal(size(LUtable),size(existingMAT.LUtable))
    error('Note from Krista: cannot proceed because seem to have new metabolites')
end
%also make sure the discrete data size matches
if ~isequal(size(discreteData),size(existingMAT.discreteData))
    error('Note from Krista: cannot proceed because have more discrete data')
end
...
%two batches of samples run in 2021
for aYear = 1:2
    %Required file #1: Go get the *mat file that resulted from one run on the TSQ
    ...
    for a = 1:size(oneRun.sInfo,1)
        ...
        for aa = 1:size(LUtable,1);
            ...
            oneMetabolite = LUtable.SRMname_2021(aa);
...
save('..\dataFiles\BIOSSCOPE_metabolites_2021.mat');
